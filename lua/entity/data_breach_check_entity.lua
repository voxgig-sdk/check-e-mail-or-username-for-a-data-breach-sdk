...
local DataBreachCheckEntity = {}
DataBreachCheckEntity.__index = DataBreachCheckEntity


function DataBreachCheckEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DataBreachCheckEntity)
  self._name = "data_breach_check"
  self._client = client
...
function DataBreachCheckEntity:get_name()
  return self._name
end


function DataBreachCheckEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DataBreachCheckEntity.new(self._client, opts)
end
...
function DataBreachCheckEntity:mark_deleted()
  self._deleted = true
end


function DataBreachCheckEntity:deleted()
  return true == self._deleted
end


function DataBreachCheckEntity:data_set(args)
  if args ~= nil then
...
function DataBreachCheckEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DataBreachCheckEntity:match_set(args)
  if args ~= nil then
...
function DataBreachCheckEntity:match_get()
...
function DataBreachCheckEntity:stream(action, args, callopts)
...
---@param reqmatch DataBreachCheckListMatch
---@param ctrl? table
---@return DataBreachCheck[]
---@return string? err
function DataBreachCheckEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function DataBreachCheckEntity:_run_op(ctx, post_done)
  local utility = self._utility
