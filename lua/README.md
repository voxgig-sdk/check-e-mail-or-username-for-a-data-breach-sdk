...
The Lua SDK for the CheckEMailOrUsernameForADataBreach API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:DataBreachCheck(nil):list(nil, nil)
...
local result, err = client:CheckEMailOrUsernameForADataBreach(nil):load(
  { id = "test01" }, nil
)
