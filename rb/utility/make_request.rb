...
module CheckEMailOrUsernameForADataBreachUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CheckEMailOrUsernameForADataBreachResponse.new({})
    ...
    if fetch_err
      ...
      response = CheckEMailOrUsernameForADataBreachResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CheckEMailOrUsernameForADataBreachResponse.new(fetched)
    else
