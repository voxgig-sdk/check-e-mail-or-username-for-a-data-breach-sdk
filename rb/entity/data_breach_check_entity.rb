...
class DataBreachCheckEntity
  ...
  def make
    opts = @_entopts.dup
    DataBreachCheckEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CheckEMailOrUsernameForADataBreachHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = CheckEMailOrUsernameForADataBreachHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [DataBreachCheckListMatch, Hash, nil] match filter (any subset of
  #   DataBreachCheck fields); defaults to nil, treated as an empty match that lists all.
