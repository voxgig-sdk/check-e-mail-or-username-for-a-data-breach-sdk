# CheckEMailOrUsernameForADataBreach Ruby SDK



The Ruby SDK for the CheckEMailOrUsernameForADataBreach API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.DataBreachCheck` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
raise a `CheckEMailOrUsernameForADataBreachError` (a `StandardError` subclass), so wrap
...
```
rb/
├── CheckEMailOrUsernameForADataBreach_sdk.rb       -- Main SDK module
