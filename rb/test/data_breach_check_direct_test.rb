...
class DataBreachCheckDirectTest < Minitest::Test
  def test_direct_list_data_breach_check
    setup = data_breach_check_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-data_breach_check", setup[:live] ? "live" : "unit")
    if _should_skip
...
def data_breach_check_direct_setup(mockres)
  ...
  if live
    ...
    client = CheckEMailOrUsernameForADataBreachSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
