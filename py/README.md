# CheckEMailOrUsernameForADataBreach Python SDK



The Python SDK for the CheckEMailOrUsernameForADataBreach API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.DataBreachCheck()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/check-e-mail-or-username-for-a-data-breach-sdk/releases)) or
...
```python
try:
    databreachchecks = client.DataBreachCheck().list({"check": "example"})
...
databreachcheck = client.DataBreachCheck().list()
...
```python
client = CheckEMailOrUsernameForADataBreachSDK.test(testopts, sdkopts)
```
...
```python
data_breach_checks = client.DataBreachCheck().list({"check": "example"})
```
...
```
py/
├── checkemailorusernameforadatabreach_sdk.py         -- Main SDK module
...
The main module (`checkemailorusernameforadatabreach_sdk`) exports the SDK class.
