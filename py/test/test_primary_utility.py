...
from checkemailorusernameforadatabreach_sdk.core.spec import CheckEMailOrUsernameForADataBreachSpec
from checkemailorusernameforadatabreach_sdk.core.result import CheckEMailOrUsernameForADataBreachResult
from checkemailorusernameforadatabreach_sdk.core.response import CheckEMailOrUsernameForADataBreachResponse
from checkemailorusernameforadatabreach_sdk.core.operation import CheckEMailOrUsernameForADataBreachOperation
...
def _make_ctx_from_map(ctxmap, client, utility):
    ...
    if isinstance(spec_map, dict):
        ctx.spec = CheckEMailOrUsernameForADataBreachSpec(spec_map)

    # Handle result from JSON map
    res_map = ctxmap.get("result")
    if isinstance(res_map, dict):
        ctx.result = CheckEMailOrUsernameForADataBreachResult(res_map)
        err_map = res_map.get("err")
        if isinstance(err_map, dict):
            msg = err_map.get("message", "")
            ctx.result.err = CheckEMailOrUsernameForADataBreachError("", msg)

    # Handle response from JSON map
    resp_map = ctxmap.get("response")
    if isinstance(resp_map, dict):
        ctx.response = CheckEMailOrUsernameForADataBreachResponse(resp_map)
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return CheckEMailOrUsernameForADataBreachError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except CheckEMailOrUsernameForADataBreachError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = CheckEMailOrUsernameForADataBreachResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_add_basic(self):
        ...
        feature = CheckEMailOrUsernameForADataBreachBaseFeature()
        utility.feature_add(ctx, feature)

        assert len(client.features) == start_len + 1

    def test_feature_hook_basic(self):
        hook_client = CheckEMailOrUsernameForADataBreachSDK.test(None, None)
        hook_utility = hook_client._utility
        ctx = _make_test_ctx(hook_client, hook_utility)

        state = {"called": False}

        class TestHookFeature(CheckEMailOrUsernameForADataBreachBaseFeature):
    ...
    def test_feature_init_basic(self):
        init_client = CheckEMailOrUsernameForADataBreachSDK.test(None, None)
        ...
        class TestInitFeature(CheckEMailOrUsernameForADataBreachBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(CheckEMailOrUsernameForADataBreachBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = CheckEMailOrUsernameForADataBreachSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = CheckEMailOrUsernameForADataBreachSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_fetch_def_basic(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = CheckEMailOrUsernameForADataBreachSpec({
            "base": "http://localhost:8080",
            "prefix": "/api",
            "path": "items/{id}",
            "suffix": "",
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = CheckEMailOrUsernameForADataBreachSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = CheckEMailOrUsernameForADataBreachSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = CheckEMailOrUsernameForADataBreachOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = CheckEMailOrUsernameForADataBreachSDK.test(None, setup_opts)
