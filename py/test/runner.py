# CheckEMailOrUsernameForADataBreach SDK test runner
...
class CheckEMailOrUsernameForADataBreachTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CheckEMailOrUsernameForADataBreachTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CheckEMailOrUsernameForADataBreachTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CheckEMailOrUsernameForADataBreachTestRunner.getenv("CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_TEST_LIVE")
        override = CheckEMailOrUsernameForADataBreachTestRunner.getenv("CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CheckEMailOrUsernameForADataBreachTestRunner.getenv(key)
        ...
        explain = CheckEMailOrUsernameForADataBreachTestRunner.getenv("CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CheckEMailOrUsernameForADataBreachTestRunner._test_control is not None:
            return CheckEMailOrUsernameForADataBreachTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CheckEMailOrUsernameForADataBreachTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CheckEMailOrUsernameForADataBreachTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CheckEMailOrUsernameForADataBreachTestRunner.load_test_control()
...
def load_env_local():
    CheckEMailOrUsernameForADataBreachTestRunner.load_env_local()


def env_override(m):
    return CheckEMailOrUsernameForADataBreachTestRunner.env_override(m)


def entity_data(v):
    return CheckEMailOrUsernameForADataBreachTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CheckEMailOrUsernameForADataBreachTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CheckEMailOrUsernameForADataBreachTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CheckEMailOrUsernameForADataBreachTestRunner.load_test_control()


def live_client_options():
    return CheckEMailOrUsernameForADataBreachTestRunner.live_client_options()


def live_delay_ms():
    return CheckEMailOrUsernameForADataBreachTestRunner.live_delay_ms()
