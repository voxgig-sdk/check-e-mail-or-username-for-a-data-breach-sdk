...
def make_error_util(ctx, err):
    if ctx is None:
        from checkemailorusernameforadatabreach_sdk.core.context import CheckEMailOrUsernameForADataBreachContext
        ctx = CheckEMailOrUsernameForADataBreachContext({}, None)
    ...
    result = ctx.result
    if result is None:
        result = CheckEMailOrUsernameForADataBreachResult({})
    result.ok = False
    ...
    errmsg = ""
    if isinstance(err, CheckEMailOrUsernameForADataBreachError):
        errmsg = err.msg
    ...
    sdk_err = CheckEMailOrUsernameForADataBreachError("", msg, ctx)
