...
//    only - a typed CheckEMailOrUsernameForADataBreachContext there would make every ctx
...
final class CheckEMailOrUsernameForADataBreachOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CheckEMailOrUsernameForADataBreachContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CheckEMailOrUsernameForADataBreachSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CheckEMailOrUsernameForADataBreachResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CheckEMailOrUsernameForADataBreachError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CheckEMailOrUsernameForADataBreachResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CheckEMailOrUsernameForADataBreachError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CheckEMailOrUsernameForADataBreachError(is_string($code) ? $code : '', $msg);
    }
