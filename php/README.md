# CheckEMailOrUsernameForADataBreach PHP SDK



The PHP SDK for the CheckEMailOrUsernameForADataBreach API — an entity-oriented client using PHP conventions.
...
try {
    ...
    foreach ($databreachchecks as $record) {
...
print_r(array_map(fn($item) => $item->data_get(), $databreachcheck));
```
...
```
php/
├── checkemailorusernameforadatabreach_sdk.php          -- Main SDK class
...
The main class (`checkemailorusernameforadatabreach_sdk.php`) exports the SDK class
