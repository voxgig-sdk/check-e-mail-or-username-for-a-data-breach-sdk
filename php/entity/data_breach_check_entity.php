...
// CheckEMailOrUsernameForADataBreach SDK DataBreachCheck entity
...
class DataBreachCheckEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DataBreachCheckEntity($this->_client, $opts);
    }

    /**
     * @param DataBreachCheck|array $args DataBreachCheck data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CheckEMailOrUsernameForADataBreachHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return DataBreachCheck|array The current DataBreachCheck data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of DataBreachCheck fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CheckEMailOrUsernameForADataBreachHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of DataBreachCheck fields).
     */
     ...
     *
     * @param DataBreachCheckListMatch|array|null $reqmatch Match filter (any subset
     *   of DataBreachCheck fields) as an assoc-array; DataBreachCheckListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return DataBreachCheck[]|array A list of DataBreachCheck items as assoc-arrays at
     *   the SDK boundary; throws CheckEMailOrUsernameForADataBreachError on failure (item-5 convention).
     */
