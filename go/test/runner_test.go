...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_TEST_LIVE") == "TRUE" ||
		os.Getenv("CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.CheckEMailOrUsernameForADataBreachSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.CheckEMailOrUsernameForADataBreachError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.CheckEMailOrUsernameForADataBreachSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.CheckEMailOrUsernameForADataBreachError{Msg: msg, Code: code}
}
