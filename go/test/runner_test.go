...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CHECKEMAILORUSERNAMEFORADATABREACH_TEST_LIVE") == "TRUE" ||
		os.Getenv("CHECKEMAILORUSERNAMEFORADATABREACH_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CHECKEMAILORUSERNAMEFORADATABREACH_TEST_EXPLAIN"); explain != "" {
