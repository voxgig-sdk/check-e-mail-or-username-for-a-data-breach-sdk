...
import (
	...
	omni "github.com/voxgig-sdk/check-e-mail-or-username-for-a-data-breach-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CheckEMailOrUsernameForADataBreachError); is {
				out := map[string]any{
					"name":    "CheckEMailOrUsernameForADataBreachError",
...
func omniCtx(arg any, client *sdk.CheckEMailOrUsernameForADataBreachSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CheckEMailOrUsernameForADataBreachSDK); is {
			client = live
