// Command CheckEMailOrUsernameForADataBreach-cli is a CLI + REPL for the CheckEMailOrUsernameForADataBreach SDK,
...
const prompt = "check-e-mail-or-username-for-a-data-breach"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "data_breach_check"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_APIKEY (api key), CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_APIKEY carries the API key and
	// CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CHECK_E_MAIL_OR_USERNAME_FOR_A_DATA_BREACH_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCheckEMailOrUsernameForADataBreachSDK(opts)
