...
//     (\`new CheckEMailOrUsernameForADataBreachSDK(...)\` / \`CheckEMailOrUsernameForADataBreachSDK.test(...)\` become
//     \`CheckEMailOrUsernameForADataBreachSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CheckEMailOrUsernameForADataBreachSDK'
...
const TEST_SEED = {"entity":{"data_breach_check":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CheckEMailOrUsernameForADataBreachSDK(...)\` and any
// \`CheckEMailOrUsernameForADataBreachSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CheckEMailOrUsernameForADataBreachSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CheckEMailOrUsernameForADataBreachSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
