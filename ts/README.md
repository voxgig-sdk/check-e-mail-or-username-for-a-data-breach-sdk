...
The TypeScript SDK for the CheckEMailOrUsernameForADataBreach API. Provides a type-safe, entity-oriented interface with full async/await support.
...
```ts
const result = await client.DataBreachCheck().list()
...
```ts
const client = new CheckEMailOrUsernameForADataBreachSDK({ apikey: '...' })
